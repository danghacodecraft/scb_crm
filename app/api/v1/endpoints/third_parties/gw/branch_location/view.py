...
from app.api.v1.endpoints.third_parties.gw.branch_location.schema import (
    SelectBranchesByRegionIdRequest, SelectBranchesByRegionIdResponse
)
...
@router.post(
    ...
    responses=swagger_response(
        response_model=ResponseData[SelectBranchesByRegionIdResponse],
        success_status_code=status.HTTP_200_OK
    )
)
async def view_gw_select_branch_by_region_id(
        request: SelectBranchesByRegionIdRequest = Body(...),
        current_user=Depends(get_current_user_from_header())
):
    gw_select_branch_by_region_id = await CtrGWBranchLocation(current_user).ctr_gw_select_branch_by_region_id(
        region_id=request.region_id
    )
    return ResponseData[SelectBranchesByRegionIdResponse](**gw_select_branch_by_region_id)
