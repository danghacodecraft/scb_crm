...
async def repos_gw_select_branch_by_branch_id(current_user: UserInfoResponse, branch_id: str):
    data_input = {
        "branch_id": branch_id
    }
    gw_select_branch_by_branch_id = await service_gw.select_branch_by_branch_id(
        current_user=current_user,
        data_input=data_input
    )
    return ReposReturn(data=gw_select_branch_by_branch_id)
