...
class PermissionController(BaseController):
    ...
    async def ctr_approval_check_permission(
        ...
        try:
            filter_code = list(filter(lambda x: dict(x).get('menu_code') == menu_code, menu_list))[0]
            filter_group_code = \
                list(filter(lambda x: x.get('group_role_code') == group_role_code, filter_code.group_role_list))[0]
        except IndexError:
            return ReposReturn(
                is_error=True,
        ...
        filter_permission_code = list(
            filter(lambda x: x.get('permission_code') == permission_code, filter_group_code.permission_list))
