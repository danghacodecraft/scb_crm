...
class Statistic(Base):
    ...
    transaction_id = Column(VARCHAR(50), nullable=False, comment="Mã giao dịch")
    full_name = Column(VARCHAR(250), nullable=False, comment="Họ và tên khách hàng")
    cif = Column(VARCHAR(7), comment="Số CIF")
    phone_number = Column(VARCHAR(10), nullable=False, comment="Số điện thoại")
    document_id = Column(VARCHAR(12), nullable=False, comment="Số GTDD")
    document_type = Column(NUMBER(2), nullable=False, comment="Loại giấy tờ")
    status = Column(VARCHAR(30), nullable=False, comment="Trạng thái")
    status_id = Column(NUMBER(2, 0), comment="ID Trạng thái")
    trans_date = Column(DateTime, comment="Ngày giao dịch")
    ekyc_step = Column(VARCHAR(50), comment="Nghiệp vụ")
    kss_status = Column(VARCHAR(30), comment="Trạng thái kiểm soát sau")
    kss_status_id = Column(NUMBER(2), comment="ID Trạng thái kiểm soát sau")
    date_kss = Column(DateTime, comment="Ngày kiểm soát sau")
    user_kss = Column(VARCHAR(15), comment="Người kiểm soát sau")
    approve_status = Column(VARCHAR(30), comment="Trạng thái phê duyệt")
    approve_status_id = Column(NUMBER(2), comment="ID Trạng thái phê duyệt")
